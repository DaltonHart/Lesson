...
In previous frameworks, we saw template strings being usded; Django also has its own! It looks a lot like template strings with javascript in that it uses a bunch of curly braces. In order to duplicate what we have on the version of Tunely, let's add the following code:
